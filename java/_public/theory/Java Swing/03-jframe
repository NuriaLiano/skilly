...
- **Operación de Cierre**: Por defecto, un JFrame no hace nada cuando el usuario intenta cerrarlo. Debes definir el comportamiento de cierre utilizando setDefaultCloseOperation(int operation), donde las operaciones comunes incluyen EXIT_ON_CLOSE (cerrar la aplicación), HIDE_ON_CLOSE (ocultar la ventana), y DISPOSE_ON_CLOSE (deshacerse de la ventana pero mantener la aplicación en ejecución).
